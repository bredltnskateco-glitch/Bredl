...
if (SECURITY_DISABLED) {
  console.warn('\n  !!  SECURITY_DISABLED=true  --  helmet, CSRF, rate-limits, CORS allowlist, mongo-sanitize, HPP, password policy, MFA & admin checks are all bypassed.');
  console.warn('  !!  See SECURITY_DISABLED.md for the re-enable checklist before deploying.\n');
}
...
for (const key of REQUIRED_SECRETS) {
  ...
  if (!SECURITY_DISABLED && key === 'JWT_SECRET' && (PLACEHOLDER_SECRETS.has(value) || value.length < 32)) {
...
const bodyLimit = SECURITY_DISABLED ? '50mb' : '100kb';
...
// (skipped automatically for safe methods, Bearer-token API clients, and
//  when SECURITY_DISABLED is true — see middleware/csrf.js).
